...
export class ThemeConfigScreen extends DynamicGameBasedScreen {
    ...
    build() {
        ...
        const viewWrapper = themeScreen.querySelector(
            ".view-wrapper",
        ) as HTMLDivElement;
        viewWrapper.appendChild(this.figuresViewer.viewer);

        const open_custom_button = themeScreen.querySelector(
            "#open-custom-screen",
        ...
        const themeRadios =
            themeScreen.querySelectorAll<HTMLDivElement>(".radio");
        themeRadios.forEach((radio) => {
            radio.onclick = () => {
                ...
                const custom_wrapper = themeScreen.querySelector(
                    "#custom-wrapper",
                ) as HTMLDivElement;

                custom_wrapper.style.display =
                    tempTheme === "custom" ? "" : "none";
        ...
        const buttons = themeScreen.querySelectorAll<HTMLButtonElement>(
            ".line-buttons > button",
        );
