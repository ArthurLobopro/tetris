...
import type { figure } from "./GameFigures";
...
export class GameController {
    #game: Game;
    private declare moveLock: boolean;
    ...
    move(direction: "right" | "left") {
        if (!this.moveLock) {
            ...
            if (
                direction === "right" &&
                !this.simulateHasBlocksOnRight(atualFigure)
            ) {
            ...
            if (
                direction === "left" &&
                !this.simulateHasBlocksOnLeft(atualFigure)
            ) {
    ...
    accelerate() {
        if (
            ...
            this.#game.tick();
            this.preventMove(
                delay(1000 / this.#game.velocities[this.#game.velocity]),
            );
        }
    }
    ...
    private simulateHasBlocksOnRight({ x, y, blocks }: blocksWithCoords) {
        return (
            x + blocks[0].length >= this.#game.width ||
            this.simulateAnyCollision({
                x: x + 1,
                y,
                blocks,
