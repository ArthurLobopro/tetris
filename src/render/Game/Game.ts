import type { PreferencesSchema } from "../../storage/StoreSchemas";
...
export class Game {
    ...
    set velocity(value: keyof typeof this.velocities) {
        ...
        if (this.fallInterval) {
            this.fallInterval.rate = this.velocities[value];
        }
    }
    ...
    verifyRecords() {
        ...
        const atualPointsIsNewRecord = records.some(
            (record) => record.points < points,
        );
        if (atualPointsIsNewRecord) {
            this.records.pop();
            const newRecordIndex = records.findIndex(
                (record) => record.points < points,
            );
